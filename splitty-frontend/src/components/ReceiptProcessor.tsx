...
const ReceiptProcessor: React.FC<ReceiptProcessorProps> = ({ imageData, onComplete }) => {
  ...
  // Use setCurrentStep directly - debug logging removed
  ...
  const selectAllFoodItems = (participant: string) => {
    const updatedItems = items.map(item => {
      if (!item.isSpecialItem) {
        ...
        if (item.qty === 1) {
          // For single quantity items, add current participant if not already included
          if (!item.shared_by.includes(participant)) {
            item.shared_by.push(participant);
          }
        } else if (item.qty > 1) {
          // For multiple quantity items, enable "split equally" and add current participant
          item.shareEqually = true;
          if (!item.shared_by.includes(participant)) {
            item.shared_by.push(participant);
          }
        }
  ...
  const calculateBill = async () => {
    if (hasUnassignedItems()) {
      ...
      setError(`Please assign all items before calculating the bill. Unassigned items: ${itemNames}`);
      return;
    }
